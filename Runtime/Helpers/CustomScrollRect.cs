...
namespace UI.Widget
{
    ...
    public class CustomScrollRect : UIBehaviour, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        private bool _draggingConsumed;
        ...
        private void Update()
        {
            _dragDelta = (Input.mousePosition - _lastMousePosition) / UnityEngine.Screen.dpi;
            _lastMousePosition = Input.mousePosition;

            if (Input.GetMouseButton(0) && Mathf.Abs(_dragDelta.y) < 0.001f)
            {
                StopMovement();
                return;
            }
        ...
        {
            if (_draggingConsumed = OnDragging(eventData.delta))
            {
                m_Velocity = Vector2.zero;
        ...
        protected virtual void LateUpdate()
        {
            ...
            {
                var deltaPos = m_Content.anchoredPosition - m_PrevPosition;
                var deltaPosDpiDependent = deltaPos.magnitude / UnityEngine.Screen.dpi * 10f;
                deltaPosDpiDependent = Mathf.Clamp(deltaPosDpiDependent, 0f, 2f);
                Vector3 newVelocity;
                if (deltaPos.magnitude / UnityEngine.Screen.dpi > 0.1f)
                {
                    newVelocity = (m_Content.anchoredPosition - m_PrevPosition) / deltaTime * scrollSpeed * deltaPosDpiDependent;
                }
                else
                {
